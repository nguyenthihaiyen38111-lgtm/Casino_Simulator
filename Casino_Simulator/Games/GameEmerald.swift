// Path: Games/GameEmerald.swift

import SwiftUI
import UIKit

struct GameEmeraldView: View {
    ...
    @State private var reel1: EmeraldReelToken = .init()
    @State private var reel2: EmeraldReelToken = .init()
    @State private var reel3: EmeraldReelToken = .init()

    @State private var lastOutcome: GameCasProfSt.SpinResolution? = nil

    @State private var winPaylines: [EmeraldPayline] = []
    ...
    private enum Assets {
        static let background = "back_castle"
        static let title = "em_title"
        static let spin = "btn_spin"
    }

    private enum Palette {
        static let blockFill = Color(hex: "BE9572")
        static let innerFill = Color(hex: "BE9572")

        static let strokeA = Color(hex: "FDE2BC")
        static let strokeB = Color(hex: "B77758")

        static let divider = Color(hex: "1C7E1C")
        static let text = Color(hex: "E9FFD8")
        static let glow = Color(hex: "C7FF93")
        static let win = Color(hex: "9BFF68")
    }
        ...
        GeometryReader { proxy in
            ...
            .onAppear {
                ...
                if reel1.finalWindow.isEmpty {
                    ...
                    reel1 = EmeraldReelToken(spins: 0, duration: 0, finalWindow: start)
                    reel2 = EmeraldReelToken(spins: 0, duration: 0, finalWindow: start)
                    reel3 = EmeraldReelToken(spins: 0, duration: 0, finalWindow: start)
                }
            }
        }
    }
        ...
        return reels
            .background(
                GeometryReader { gp in
                    Color.clear
                        .overlayPreferenceValue(EmeraldSymbolBoundsPreferenceKey.self) { anchors in
                            EmeraldPaylineGlowBehindSymbols(
                                anchors: anchors,
    ...
    private func spin() {
        ...
        reel1 = EmeraldReelToken(spins: spinsCount(for: r1), duration: r1, finalWindow: w1)
        reel2 = EmeraldReelToken(spins: spinsCount(for: r2), duration: r2, finalWindow: w2)
        reel3 = EmeraldReelToken(spins: spinsCount(for: r3), duration: r3, finalWindow: w3)
    ...
    private func computeAndShowPaylines(rows: Int, c0: [SlotSymbol], c1: [SlotSymbol], c2: [SlotSymbol]) {
        let grid = EmeraldSlotGrid(rows: rows, col0: c0, col1: c1, col2: c2)
        let all = EmeraldPayline.defaults(forRows: rows)
    ...
    private func startPaylinesLoop(grid: EmeraldSlotGrid) {
        let id = UUID()
        paylinesLoopId = id
    ...
    private func showWinBanner(for grid: EmeraldSlotGrid, paylineIndex: Int) {
    ...
    private func comboTitle(for symbol: SlotSymbol, multiplier: Double) -> String {
        if multiplier >= 50 { return "CASTLE JACKPOT x50" }
        if multiplier >= 10 { return "CASTLE x10" }
        if multiplier >= 3 { return "CASTLE COMBO x3" }
        if multiplier >= 1.5 { return "CASTLE WIN" }

        switch symbol {
        case .three: return "Crystals"
        case .bell: return "Money Bags"
        case .strawberry: return "Green Crystal"
        case .cherry: return "Crystal Duo"
        case .watermelon: return "Crystal Core"
        case .grape: return "Crystal Bag"
        }
    }
...
private struct EmeraldReelToken: Equatable {
    let id: UUID
    let spins: Int
...
private struct EmeraldSymbolCellKey: Hashable {
    let reel: Int
    let row: Int
}

private struct EmeraldSymbolBoundsPreferenceKey: PreferenceKey {
    static var defaultValue: [EmeraldSymbolCellKey: Anchor<CGRect>] = [:]

    static func reduce(value: inout [EmeraldSymbolCellKey: Anchor<CGRect>], nextValue: () -> [EmeraldSymbolCellKey: Anchor<CGRect>]) {
        value.merge(nextValue(), uniquingKeysWith: { $1 })
    }
}

private struct EmeraldPaylineGlowBehindSymbols: View {
    let anchors: [EmeraldSymbolCellKey: Anchor<CGRect>]
    let paylines: [EmeraldPayline]
    let activeIndex: Int
    let isWinning: Bool
    ...
    private enum Palette {
        static let glow = Color(hex: "FDE2BC")
        static let core = Color(hex: "B77758")
        static let win = Color(hex: "FDE2BC")
    }
            ...
            ZStack {
                if let p = activePayline, p.rowsByReel.count == 3 {
                    ...
                    ForEach(0..<3, id: \.self) { reel in
                        ...
                        let key = EmeraldSymbolCellKey(reel: reel, row: row)
    ...
    private var activePayline: EmeraldPayline? {
        guard !paylines.isEmpty, paylines.indices.contains(activeIndex) else { return nil }
        return paylines[activeIndex]
    }
}

private struct EmeraldReelView: View {
    let reelIndex: Int
    @Binding var token: EmeraldReelToken
    ...
    private func loopStrip(step: CGFloat, offset: CGFloat) -> some View {
        ...
        VStack(spacing: rowGap) {
            ForEach(Array(symbols.enumerated()), id: \.offset) { _, sym in
                Image(EmeraldSymbolMapper.imageName(for: sym))
                    .resizable()
                    .scaledToFit()
        ...
        VStack(spacing: rowGap) {
            ForEach(Array(fw.enumerated()), id: \.offset) { row, sym in
                Image(EmeraldSymbolMapper.imageName(for: sym))
                    ...
                    .scaledToFit()
                    .frame(width: symbolSize, height: symbolSize)
                    .anchorPreference(key: EmeraldSymbolBoundsPreferenceKey.self, value: .bounds) { a in
                        [EmeraldSymbolCellKey(reel: reelIndex, row: row): a]
                    }
            }
        }
...
private enum EmeraldSymbolMapper {
    static func imageName(for symbol: SlotSymbol) -> String {
        switch symbol {
        case .strawberry: return "crown"
        case .cherry: return "flag"
        case .watermelon: return "gear_wheel"
        case .grape: return "shield_1"
        case .bell: return "shield"
        case .three: return "magnifier"
        }
    }
}

private struct EmeraldSlotGrid {
    let rows: Int
    let col0: [SlotSymbol]
    let col1: [SlotSymbol]
    let col2: [SlotSymbol]
...
private struct EmeraldPayline: Identifiable, Equatable {
    let id: String
    let rowsByReel: [Int]

    static func defaults(forRows rows: Int) -> [EmeraldPayline] {
        let safeRows = max(1, rows)
        let top = EmeraldPayline(id: "top", rowsByReel: [0, 0, 0])
        let center = EmeraldPayline.center(forRows: safeRows)
        let mid2 = EmeraldPayline(
            id: "mid2",
            rowsByReel: [min(2, safeRows - 1), min(2, safeRows - 1), min(2, safeRows - 1)]
        )
        let bottom = EmeraldPayline(id: "bottom", rowsByReel: [safeRows - 1, safeRows - 1, safeRows - 1])

        let diagDown = EmeraldPayline(id: "diagDown", rowsByReel: [0, 1, 2].map { min(max($0, 0), safeRows - 1) })
        let diagUp = EmeraldPayline(id: "diagUp", rowsByReel: [2, 1, 0].map { min(max($0, 0), safeRows - 1) })

        return [top, center, mid2, bottom, diagDown, diagUp]
    }

    static func center(forRows rows: Int) -> EmeraldPayline {
        let r = max(1, rows)
        let center = min(r - 1, 1)
        return EmeraldPayline(id: "center", rowsByReel: [center, center, center])
    }

    func isWin(in grid: EmeraldSlotGrid) -> Bool {
