// Path: Games/GameFish.swift

import SwiftUI
import UIKit

struct GameFishView: View {
    ...
    @State private var reel1: FishReelToken = .init()
    @State private var reel2: FishReelToken = .init()
    @State private var reel3: FishReelToken = .init()

    @State private var lastOutcome: GameCasProfSt.SpinResolution? = nil

    @State private var winPaylines: [FishPayline] = []
    ...
    private enum Assets {
        static let background = "fish_back"
        static let title = "fish_title"
        static let spin = "btn_spin"
    }

    private enum Palette {
        static let blockFill = Color(hex: "00A9F8")
        static let innerFill = Color(hex: "007FC1")

        static let strokeA = Color(hex: "00306D")
        static let strokeB = Color(hex: "F8FFFE")

        static let divider = Color(hex: "00306D")
        static let text = Color(hex: "F8FFFE")

        static let glow = Color(hex: "BFEFFF")
        static let win = Color(hex: "F8FFFE")
    }
        ...
        GeometryReader { proxy in
            ...
            .onAppear {
                ...
                if reel1.finalWindow.isEmpty {
                    ...
                    reel1 = FishReelToken(spins: 0, duration: 0, finalWindow: start)
                    reel2 = FishReelToken(spins: 0, duration: 0, finalWindow: start)
                    reel3 = FishReelToken(spins: 0, duration: 0, finalWindow: start)
                }
            }
        }
    }
        ...
        return reels
            .background(
                GeometryReader { gp in
                    Color.clear
                        .overlayPreferenceValue(FishSymbolBoundsPreferenceKey.self) { anchors in
                            FishPaylineGlowBehindSymbols(
                                anchors: anchors,
    ...
    private func spin() {
        ...
        reel1 = FishReelToken(spins: spinsCount(for: r1), duration: r1, finalWindow: w1)
        reel2 = FishReelToken(spins: spinsCount(for: r2), duration: r2, finalWindow: w2)
        reel3 = FishReelToken(spins: spinsCount(for: r3), duration: r3, finalWindow: w3)
    ...
    private func computeAndShowPaylines(rows: Int, c0: [SlotSymbol], c1: [SlotSymbol], c2: [SlotSymbol]) {
        let grid = FishSlotGrid(rows: rows, col0: c0, col1: c1, col2: c2)
        let all = FishPayline.defaults(forRows: rows)
    ...
    private func startPaylinesLoop(grid: FishSlotGrid) {
        let id = UUID()
        paylinesLoopId = id
    ...
    private func showWinBanner(for grid: FishSlotGrid, paylineIndex: Int) {
    ...
    private func comboTitle(for symbol: SlotSymbol, multiplier: Double) -> String {
        if multiplier >= 50 { return "OCEAN JACKPOT x50" }
        if multiplier >= 10 { return "DEEP SEA x10" }
        if multiplier >= 3 { return "FISH COMBO x3" }
        if multiplier >= 1.5 { return "FISH WIN" }

        switch symbol {
        case .three: return "Big Catch"
        case .bell: return "Lucky Fisherman"
        case .strawberry: return "Blue Fish"
        case .cherry: return "Shark Strike"
        case .watermelon: return "Rod Catch"
        case .grape: return "Jelly Combo"
        }
    }
...
private struct FishReelToken: Equatable {
    let id: UUID
    let spins: Int
...
private struct FishSymbolCellKey: Hashable {
    let reel: Int
    let row: Int
}

private struct FishSymbolBoundsPreferenceKey: PreferenceKey {
    static var defaultValue: [FishSymbolCellKey: Anchor<CGRect>] = [:]

    static func reduce(value: inout [FishSymbolCellKey: Anchor<CGRect>], nextValue: () -> [FishSymbolCellKey: Anchor<CGRect>]) {
        value.merge(nextValue(), uniquingKeysWith: { $1 })
    }
}

private struct FishPaylineGlowBehindSymbols: View {
    let anchors: [FishSymbolCellKey: Anchor<CGRect>]
    let paylines: [FishPayline]
    let activeIndex: Int
    let isWinning: Bool
    ...
    private enum Palette {
        static let glow = Color(hex: "DDF8FF")
        static let core = Color(hex: "00A9F8")
        static let win = Color(hex: "F8FFFE")
    }
            ...
            ZStack {
                if let p = activePayline, p.rowsByReel.count == 3 {
                    ...
                    ForEach(0..<3, id: \.self) { reel in
                        ...
                        let key = FishSymbolCellKey(reel: reel, row: row)
    ...
    private func loopStrip(step: CGFloat, offset: CGFloat) -> some View {
        ...
        VStack(spacing: rowGap) {
            ForEach(Array(symbols.enumerated()), id: \.offset) { _, sym in
                Image(FishSymbolMapper.imageName(for: sym))
                    .resizable()
                    .scaledToFit()
        ...
        VStack(spacing: rowGap) {
            ForEach(Array(fw.enumerated()), id: \.offset) { row, sym in
                Image(FishSymbolMapper.imageName(for: sym))
                    ...
                    .scaledToFit()
                    .frame(width: symbolSize, height: symbolSize)
                    .anchorPreference(key: FishSymbolBoundsPreferenceKey.self, value: .bounds) { a in
                        [FishSymbolCellKey(reel: reelIndex, row: row): a]
                    }
            }
        }
...
private enum FishSymbolMapper {
    static func imageName(for symbol: SlotSymbol) -> String {
        switch symbol {
        case .strawberry: return "fish_1"
        case .cherry: return "shark"
        case .watermelon: return "fishing_rod"
        case .grape: return "jellyfish"
        case .bell: return "fisherman"
        case .three: return "fish_c"
        }
    }
}

private struct FishSlotGrid {
    let rows: Int
    let col0: [SlotSymbol]
    let col1: [SlotSymbol]
    let col2: [SlotSymbol]
...
private struct FishPayline: Identifiable, Equatable {
    let id: String
    let rowsByReel: [Int]

    static func defaults(forRows rows: Int) -> [FishPayline] {
        let safeRows = max(1, rows)
        let top = FishPayline(id: "top", rowsByReel: [0, 0, 0])
        let center = FishPayline.center(forRows: safeRows)
        let mid2 = FishPayline(id: "mid2", rowsByReel: [min(2, safeRows - 1), min(2, safeRows - 1), min(2, safeRows - 1)])
        let bottom = FishPayline(id: "bottom", rowsByReel: [safeRows - 1, safeRows - 1, safeRows - 1])

        let diagDown = FishPayline(id: "diagDown", rowsByReel: [0, 1, 2].map { min(max($0, 0), safeRows - 1) })
        let diagUp = FishPayline(id: "diagUp", rowsByReel: [2, 1, 0].map { min(max($0, 0), safeRows - 1) })

        return [top, center, mid2, bottom, diagDown, diagUp]
    }

    static func center(forRows rows: Int) -> FishPayline {
        let r = max(1, rows)
        let center = min(r - 1, 1)
        return FishPayline(id: "center", rowsByReel: [center, center, center])
    }

    func isWin(in grid: FishSlotGrid) -> Bool {
